...
const defaultMockPath = path.resolve(process.cwd(), 'mock');
const pathSplit = path.sep; // 目录分隔符： windows: \\ linux: /

module.exports = function (req, res, next, options={}) {
  const mockPath = options.mockPath || defaultMockPath;
  ...
  if (handlerFile) {
    ...
    setTimeout(()=>{
      let reqDelay = req.body.delay || req.query.delay || delay;//延迟时间
        ...
        if (handlerFile.match(/\.js$/)) {
          ...
          if (typeof handlerMod == 'function') {
            ...
            if (handlerMod.successReturn || handlerMod.errorReturn) {
              ...
              // 普通对象
              //a js module to return object, it's then response.
    ...
    // can'nt match any file in the mockData path; it will go ahead to next()
    next();
  }
}
...
function tryMatchModules(reqFullPath) {
  ...
  if (!matchRet.length) {
    ...
    // both the js and json file exits, it prefers to use js file;

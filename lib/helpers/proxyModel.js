...
const { dynamicProxys, defautSetting } = require('../config');
    ...
    .then(port=>{
      ...
      let serverInstance = app.listen(port, err=>{
        if (!err) {
          ...
          let proxyFn = createProxyMiddleware(proxyConfig);
          // 代理到内建http服务器
...
function tryProxyRemote(req, res, next, proxyConfig={}){
  ...
  /** switch different enviroment by host */
  ...
  let proxyFn = createProxyMiddleware(config);
  // 代理到远程http服务器
  proxyFn(req, res, next);
}
...
/**动态代理中间件*/
function proxyRemoteService(req, res, next) {
  ...
  let proxy = proxies.find(prox => {
    ...
    if ((new RegExp(`^${prox.proxyPath}`.replace(/^[\^]+/, '^'))).test(fullPath)) {
      return true;
    }
    if (fullPath.indexOf(prox.proxyPath) === 0) {
      return true;
    }
  });
  if (proxy) {
    console.log(`\n`);
    console.log(`********** 命中代理规则 **********`);
    console.log(`proxyPath: ${proxy.proxyPath}`)
